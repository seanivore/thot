# v5.0.0 Implementation Plan — Thot

**Initiative**: The v5 public-launch initiative. Thot's first public release (v5.0) is gated on the app becoming a markdown *and* normal-text editor with proprietary semantic highlighting. This document is the consolidated whole-project roadmap — every feature area from the shipped v4.1.0 baseline, through the v4.2.0 polish, the v5.0 moat (proprietary highlight scope + Intelligent Formatting dual-mode), the v6.0 preferences UI, and the vNext native / collaboration / AI horizon.
**Version**: v5.0.0
**Revision driven by**: Initial consolidation (2026-05-20). The v1–v4 planning archives and the 2026-05-20 session notes were purged into this single feature/behavior map; the two research directories were merged into `docs/research/`; source-provenance citations were stripped; the whole document was reorganized by feature area. Supersedes `v4_1_0_IMPLEMENT.md` as the active living roadmap.
**Date opened**: 2026-05-20
**Status**: Planning. The feature/behavior map is consolidated and organized; individual sections are iterating toward exclusively-executable. Not build-ready as a whole — v4.2.0 (Heading Stacking) is the one section carrying a locked, BUILD-ready spec.

**Required reading first**:
  - `docs/THOT_APP.md` — architecture and current state
  - `README.md` — project status, deploy URLs, commands
  - `.agent/DEV_RULES.md` — process, file types, the gap-finding loop

**If you find missing context as you read this**: `docs/THOT_APP.md` is the living architecture doc. Confirm the gap with Sean and update `THOT_APP.md`; don't paper over it inside this document.

---

## How This Document Works

This is the project's living roadmap — the highest-numbered `vX_Y_Z_IMPLEMENT.md`, covering the whole project rather than one milestone. It is organized **by feature area** (Editor Foundation, Semantic Highlighting, Persistence, …), not by version milestone; each feature section carries a **status tag** that places it on the version timeline.

It is an *in-progress* IMPLEMENT. Per the gap-finding loop (`.agent/DEV_RULES.md`), open questions, `(Concept, needs research)` items, and `**CONFLICT**` markers are expected at this stage — they are the work still ahead. Only a chunk promoted to a `BUILD` packet must be fully locked and decision-free.

This document is **self-contained**. The v1–v4 planning archives (`docs/archive/v1_0/` through `docs/archive/v4_0/`) and all prior session notes were fully folded into it during the 2026-05-20 consolidation — no agent needs to read them; they remain in `docs/archive/` only as historical record. Everything needed to understand or extend the plan is here, in `docs/THOT_APP.md`, or in the live research under `docs/research/`.

**Status tags** on feature headings:
  - **(Shipped v4.1.0)** — live in production
  ...
  - **(Cancelled — see X)** — attempted and abandoned; rationale preserved so it is not re-litigated
  - **(Speculative — future watching)** — long-horizon ideas worth keeping on the radar

`**CONFLICT**` markers flag unresolved disagreements to decide in a future pass. `→ Research:` links point to live research under `docs/research/`. Section-local `### Open Questions` subsections roll up into the closing "Open Questions & Unresolved Designs" appendix.

A companion outline — `v5_0_IMPLEMENT_OUTLINE.md`, in this directory — maps every section and subsection to its line number. Regenerate it whenever this document changes substantially.
...
| ------------- | ---------------------------------------------------------------- | ------------------- | ------------------------------------------------- |
| **v4.0.x**    | Round-out-the-basics polish                                      | Internal            | Shipped 2026-05-11                                |
| **v4.1.0**    | URL clickability + anchor links                                  | Internal            | Shipped 2026-05-11                                |
| **v4.2.0**    | Heading stacking (sticky-heading scroll)                         | Internal            | Spec locked — ready to promote to a BUILD packet  |
| **v5.0.0**    | Proprietary highlight scope + Intelligent Formatting dual-mode    | **Public soft launch** | Architecture researched; spec drafting         |
| **v6.0.0**    | User preferences UI on the v5 scope system                       | Public              | Planned                                           |
| **vNext**     | Native wrappers, real-time collaboration, AI, columns/workspace  | Public + App Store  | Strategy phase — direction, not commitment        |
| **Parallel**  | Business-plan kickoff (non-version track)                        | Internal            | Research consolidated; first session ready        |

**Public-release line**: Thot does not ship publicly until **v5.0** — the point at which it is a markdown *and* normal-text editor with semantic highlighting. v4.x is internal / early-user only.

### Research status snapshot

Locked research recommendations (ready for IMPLEMENT planning when the build queue reaches them):
  - **Highlighter scope-system rewrite (v5 moat)** → `docs/research/1_DEEP/highlighter-architecture/OPTIONS.md` — Option A (stay on Lezer, rewrite the scope layer) + Option B2 (prose-mode regex highlighter).
  - **Native wrappers** → `docs/research/1_DEEP/native-wrapper/OPTIONS.md` — Capacitor shell + native Swift for Live Activities / Pencil / WatchOS / widgets; phasing A–F.
  - **Auth + sync** → `docs/research/1_DEEP/auth-and-sync/OPTIONS.md` — Clerk for auth, Postgres for v1 sync, Yjs + Liveblocks for v2 collaboration; `VERIFY` tags need a Phase-2 pass.

Scaffolds awaiting Sean's narrative fill-in:
...
Seed buckets — research not yet started:
  - Real-time collaboration → `docs/research/1_DEEP/collab-for-markdown/1_REVIEW.md`
  - AI features → `docs/research/1_DEEP/ai-features/1_REVIEW.md`
  - IDE feature parity → `docs/research/1_DEEP/ide-feature-parity/1_REVIEW.md`
  - Monetization *(business track)* → `docs/research/business/monetization/1_REVIEW.md`
  - Market positioning *(business track)* → `docs/research/business/market-positioning/1_REVIEW.md`
...
**Standing design constraint — every web setting must translate to native.** This is stated prominently and deliberately so every agent keeps it in mind across the whole document: EVERY setting built for the web app must translate cleanly onto the native macOS / iPadOS / iOS preference surfaces when the web build is "dropped" into the native wrapper. The connector that maps web-app settings onto native preference panes lives in the wrapper; it must auto-update the instant a new web build is dropped in, and it must stay simple — ideally it picks up new settings automatically rather than needing hand-wiring per setting. The practical rule: whenever ANY setting or preference is specified anywhere in this document, its native-translation path must be considered as part of the spec, not deferred. The web view may look different from the native surface, but the bridge that connects the two should never become a place where settings drift or rot.
...
**The explicit goal**: a user of VS Code, Cursor, or Antigravity should be able to use Thot *instead of* their IDE for markdown work. That sets a research obligation — study what IDE features that switch actually requires, feature by feature, rather than guessing. Concrete IDE conveniences already identified for study live as feature items in the relevant sections (file-explorer git-status indicators, settings import from VSC forks, repo mounting from GitHub or a local directory — see § File Management & I/O and § Workspace & Multi-Note).

**Market validation — IDE-adjacent-but-separate is a real category.** Google's Antigravity now ships a *separate* IDE app alongside its agent-focused app. When a company at Google's scale concludes that the agent-centric tool and the IDE belong in two distinct apps, it validates the thesis that an IDE-adjacent-but-separate app is a genuine product category — which is exactly the space Thot targets for markdown work.

**A prime target market — claude.ai/code users.** Users running instances on claude.ai/code are a prime target: they often work from a device that either cannot run, or simply doesn't have, a full IDE. With the web Thot they could edit markdown without ever leaving the browser. The concrete personal use case driving this is editing markdown on an iPad in bed — and both the web app and the native apps need to support that scenario well.
...
The goal is concretely stated as: *open a markdown file in Cursor and Thot side-by-side; they feel visually equivalent in semantics, hierarchy, highlight color, spacing.*
...
**The full 9-weight family loaded**:
...
**Per-element weight assignments**:
...
| Headings size | Same as body                    | Intentional design choice; headings differentiated by color and weight, not size. |

Sean's v1 framing held that line spacing should be *modestly relaxed (e.g., 1.2–1.4) for readability in dense notes*. The shipped value (1.5) lands slightly above that initial range — confirmed in v2.1.4 as the final pick after testing the original 1.0 felt cramped.
...
> HEADINGS — I changed the headings so that they're the same size as the rest of the document. Please leave them this way.

Codified in docs/THOT_APP.md § Common Pitfalls:

> **Headings are the same font size as body text** — Intentional design choice by the user — headings distinguished by color and weight, not size.
...
#### Color Palette Snapshot **(Shipped v4.1.0)**
...
#### Highlight Priority Hierarchy **(Shipped v4.1.0)**
...
> Also, we should change the highlight color because it is this very transparent faded green on top of the charcoal background so when text is highlighted, it is very difficult to see if it highlighted the whole word or not on mobile especially.

**(Spec'd, not shipped)** — Selection color is an outstanding usability tweak; tracked but not yet implemented.

### Line Wrapping

#### `EditorView.lineWrapping` **(Shipped v4.1.0)**

Soft-wrap is **unconditional** — `EditorView.lineWrapping` is included in the extensions array unconditionally at `src/editor.ts:303`. The Phase 0 v4.0.0 DevTools spike was to investigate a suspected "wrap stops below 1040px viewport" bug. Outcome:

> Static analysis of `node_modules/@codemirror/view/dist/index.js:6608–6640` confirmed CM's `.cm-content.cm-lineWrapping { white-space: break-spaces; ... }` rule fires unconditionally — `EditorView.lineWrapping` in our extensions adds the class. No CSS override below 1040px exists in our code or CM base. Conclusion: the bug does not reproduce on current code. No fix applied. Sean confirmed wrap works at narrow viewport on the dev preview.

So wrap is shipped and behaving correctly across viewport widths. The v1 framing — *Hard wraps at view width (no horizontal scrolling in normal usage)* — is honored.

#### Hanging Indent / Wrapped-Line Indentation **(Shipped v4.1.0)**

**Rationale, from v2.0.0 feedback** (verbatim, this is Sean's voice):

> When typing in a block of text — standard paragraph or either types of lists with really long items that wrap — the wrapped next lines must adhere to the indentation at which the first line in the block of text started. Let me know if you need pictures of what I'm talking about. This is **NOT** how a normal word processor would work which is maybe why it doesn't, but since markdown is traditionally written in an IDE, the text is treated more like code.
...
The paired-delimiter behavior is the single feature Sean has called out as *"a feature I use CONSTANTLY and would love it in this update."*

#### `closeBrackets` for type-an-opener **(Shipped v4.1.0)**
...
This was specified back in v1 and was the v1 Swift implementation in `PairedDelimiterTextView`:

> Pairs are inserted correctly. Typing closing character skips over existing closing pair. Backspace between empty pair deletes both. Text selection + opening char wraps selection. Smart quote handling (no pair mid-word for contractions). Tab inserts 2 spaces.

Validated in v3 usability testing:

> Typing any delimiter creates both, like it should. Highlighting a word and then typing either `"` or `'` places the double or single quotes around the word, as expected. Highlighting a word and then typing `(` or `{` or `[` all places the set of delimiters around the word, as expected.

#### Shift-Marker Wrapping — markdown syntax wrapping **(Spec'd, not shipped)**
...
> Adding delimiters to a highlighted word only half works. However, none of the other delimiters can be applied by highlighting and tapping the delimiter. This should also work for adding `inline code markers to a word`, *italics* and **bold** asterisks, `<carrots>`, or ~~strikeout~~.
...
The original v1 spec noted *Smart quote handling (no pair mid-word for contractions)*. The web build's `closeBrackets` does NOT currently do mid-word suppression — typing `dont` then `'` between the t and end would auto-pair. This is acceptable for an IDE-style markdown editor (the autocorrect engine corrects `dont` → `don't` separately), but Sean has flagged context-aware quote pairing as desirable in the long run.

### Autocorrect Engine

The autocorrect engine is one of Thot's most user-noticeable behaviors and is implemented as a fully-proprietary system distinct from native browser autocorrect. The shipped behavior in v3.1.0+ is the result of an extensive set of refinements.

#### Architecture: StateField + Transaction Filter Pattern **(Shipped v4.1.0)**
...
#### Backspace-Revert Behavior **(Shipped v4.1.0)**
...
#### Suppression Contexts (Syntax-aware) **(Shipped v4.1.0)**
...
**Known gap**: the `->` dictionary entry was added but the regex doesn't include `>` in its character class, so `->` never fires the substitution. A `[a-zA-Z0-9_:-]+` regex does not capture `->` because `>` isn't in the character class. The dictionary entry was added but the regex fix was deliberately deferred at Sean's call (skip non-sure-wins; revisit in v4.0.1). The `->` token does not currently autocorrect.
...
**Open consideration — the arrow-glyph tension.** What Sean actually wants from `->` is the clean, short arrow *glyph* that IDE editors render — the arrow ligature you see in VS Code / Cursor. But Thot deliberately ships JetBrains Mono **NL** (the no-ligature variant) precisely because ligatures interfere with markdown semantics, where the literal characters are part of the document content (see § Typography § Font Family). The autocorrect `->` → `→` substitution is the current answer to this — it produces a real arrow *character* in the source text rather than a font-rendering trick — but it doesn't fully match the underlying want, which is the IDE-quality arrow look without the literal-character cost. This stays an unresolved tension/preference: the substitution is the shipped compromise, the IDE arrow ligature is the thing Sean would actually reach for, and the two cannot both be true while the no-ligature font choice holds.

#### Auto-Capitalization on Sentence Starts **(Shipped v4.1.0)**

Sentence-start auto-capitalization is bundled into the same autocorrect engine. After a sentence terminator (`.`, `!`, `?`) followed by space(s), the next typed alphabetic character is capitalized. Same suppression rules apply (no capitalization inside the `InlineCode`, `FencedCode`, `URL`, `Link` contexts as word-level autocorrect).

Sean's mobile feedback flagged this as critical:

> most notable is of course the lack of the initial cap for a new sentence. maybe this is just turned off because it is a "code" editor and we just happen to be using it more specifically for word processing. regardless. very necessary.

This is shipped via the custom autocorrect engine. The desktop / iOS native browser autocapitalize attributes are explicitly **off** (see Mobile-Specific UX § iOS Predictive Text Disabled) because they fight with the custom engine and trigger the iOS predictive-text bar.

#### The Dictionary — Full Reference **(Shipped v4.1.0)**
...
#### v6 Preferences-UI Integration **(Spec'd, not shipped — v6 target)**

The v6 preferences UI will expose:

- **Add word to dictionary** (right-click context menu extension over OS-default)
- **Create Autocorrect Rule** — right-click context menu item that opens the preferences modal at the Autocorrect Rules section. If the selected word is already in the dictionary, the **target** field — "the word you want to autocorrect TO" — is pre-filled. If the selected word is not in the dictionary, the **trigger** field — "the word you want to autocorrect" — is pre-filled.
...
#### `spellcheck="true"` via `contentAttributes` **(Shipped v4.1.0)**
...
#### Toggle Shortcut — `Mod-Shift-c` **(Shipped v4.1.0)**
...
#### Known Mobile Gap **(Shipped, with known gaps)**
...
### Destructive-Action Confirmation **(Spec'd, not shipped)**

The "new / clear pad" action — clearing the current content to start fresh — should show a confirmation prompt before it runs. Today the action is destructive with no guard, so a stray click or shortcut can nuke the pad's entire content with no recovery beyond CMD+Z. A simple confirm dialog ("Clear this pad?") makes the action deliberate without adding friction to normal writing.

### First-Run / Empty-State Content **(Shipped v4.1.0)**

On first load with no saved content, Thot does not show a blank pad — it shows welcome/intro markdown. The empty state is therefore content, not emptiness: a brief introduction rendered as markdown so a brand-new user sees the editor working (highlighting, hierarchy, the dark theme) immediately rather than facing an empty surface. Once the user has saved content, the saved content loads as normal and the intro never reappears.

### Box-Drawing Character Insertion **(Concept, needs research)**

Architecture, technical, and AI-generated docs frequently use box-drawing / tree / flow-chart characters — `│ ─ ├ ┼ ┌ └ ┐ ┘ ┤` and the like — the kind that show up in ASCII directory trees and diagrams. These are awkward to type because they aren't on the keyboard. Thot should offer a quick-insert affordance for them: a command or shortcut that surfaces the box-drawing set so the user can drop the right glyph without hunting a character picker. A candidate binding mentioned in sources is `OPTION+COMMAND+RIGHT ARROW`, but the exact mechanism is open — a palette, a contextual menu, or a keyed cycle through the set are all on the table. This is distinct from AI diagram-cleanup-on-save (already covered in § AI Integration); this feature is about *creating* these characters by hand, not tidying a diagram an LLM produced.
...
| `Tab`                                    | Insert 2 spaces (no smart-list logic)                           | v1 spec carried forward |
...
#### `Mod-s` Force-Save Intercept **(Shipped v4.1.0)**
...
> Right now though it would be great if we could make it so that CMD + S just nudges a normal auto-save. Right now it opens a window more like save-as and you obviously can only save as HTML. My main reason for mentioning this is because I compulsively hit CMD + S and it opens a window every time.
...
#### Custom Enter Handler — Exit List on Empty Bullet **(Shipped v4.1.0, with documented quirk)**
...
> Desired model: Enter after content = new bullet. Enter on empty bullet = exit list. Manual blanks don't persist as auto-spacing.
...
| 2. Multi-item list with manual blank lines: typing Enter after second item auto-adds blank before new bullet (should NOT auto-propagate). | **Spec'd, not shipped** — explicitly deferred to v4.0.1 BUGS off-ramp. Likely entangled with `indentOnInput`; may need different fix path. |
| 3. Desired model fully enforced.                                                                                                          | **Partially shipped** — quirks 1 fix delivers most of model; quirk 2 fix delivers the rest.                                                               |

v4.0.0 testing confirmed: empty-bullet Enter strips the marker; quirk 2 was not triggered in tests.
...
#### Root Cause Analysis (for future fix sessions)
...
### Exception: Rejoining Sub-Lists **(Shipped v4.1.0)**
...
### Paired-Delimiter Gap for Markdown Syntax Wrapping **(Spec'd, not shipped)**

Bullet/number list items are the context where Sean most often wants markdown-marker wrapping (highlight a word, tap `*`/`~`/etc.) to fire. The full spec lives in Editor Foundation § Shift-Marker Wrapping.
...
| Checkbox `[x]` checked        | `#5A7DB8` (darker blue) | Regular (400) | **Spec'd in v4.0.0 — does NOT visibly render** due to cascade specificity. Deferred to v5 scope rebuild. |

### List Content Bleed Onto Following Lines **(Shipped v4.1.0, partial fix)**
...
**Side effect (accepted by Sean)**:

> Tight lists (no blank lines between items) don't wrap inline content in `Paragraph`, so their list-item text lost the cyan/pink content tag. Sean's explicit call: the bleed fix is the priority; the inline-color side effect is acceptable v5 work, do not revert.
...
**Scope note on the original wording**: the original source described list *"formatting"* bleeding onto following lines — not only list *color*. The current spec deliberately scopes this entry to color-tag bleed (the cyan/pink content tag); the broader wording is acknowledged here so the narrowing is a conscious, recorded choice rather than an oversight. If any non-color list formatting (weight, style) is later found to bleed similarly, it belongs under this same heading.

### Frontmatter Mis-Tagging in List Contexts **(Cancelled — see v5 scope rebuild)**

This is how the frontmatter mis-tagging bug shows up specifically inside list contexts: in a numbered list, hit return for sub-bullet → delete auto-number → type `-` and space → the hyphen renders in the orange/bold frontmatter style. The root-cause analysis (the inert `DocumentMeta` remap, the SetextHeading2 promotion, and the v5 fix decision) lives in § Semantic Highlighting & Theme § Frontmatter / DocumentMeta Workaround (v4.0.0.3).
...
The digit-responsive spacing behavior applies on **all devices** — it was originally flagged as a "mobile layout" issue, but Sean clarified it is notable on desktop too: *"The issue with the numbered lines nudging over with each new number is ALL DEVICES."* Mobile and desktop share the exact same gutter behavior.
...
### Shipped Implementation **(Shipped v4.0.0)**
...
**`src/theme.ts`** — `.cm-lineNumbers .cm-gutterElement` rule (final shipped form, after commits 44c6529 + 92c1868):
...
The Phase 2 work only tokenized `width`. Two regressions surfaced and were fixed in follow-up commits:
...
Sean confirmed perfect across the 99→100→101 boundary.
...
### Collapsing Range of Lines **(Spec'd, not shipped)**
...
### Colored Line Number Indicator — Edit/Add State **(Spec'd, not shipped)**
...
> We don't yet have a mobile test environment yet. Should note that iPad is way more important than mobile.
...
### Bottom Padding — `padding-bottom: 30vh` **(Shipped v4.1.0)**
...
**Future refinement — dynamic auto-grow buffer**: the shipped behavior is a static `padding-bottom: 30vh`. The original spec wanted something more refined: a dynamic, gradual incremental buffer that auto-grows as the cursor nears the bottom of the document — the way Google Docs gives you "room to keep typing" rather than a fixed dead zone. This is a documented future refinement, not a contradiction of the shipped behavior. Shipped = static 30vh; desired refinement = dynamic auto-grow as the cursor approaches the bottom.

### iOS Safari Initial-Viewport Miscalculation **(Shipped, with known gaps)**

On iOS Safari, the editor does not compute its viewport correctly on initial load. Symptoms: the document extends below the bottom of the screen, pull-to-refresh stops short (around line 4), and the full content only appears after the user taps into the text area. The likely culprit is CSS viewport-units behavior (`vh` on iOS not accounting for the dynamic browser chrome) combined with touch-event handling that doesn't settle the layout until first interaction. Tracked as a known bug; the fix probably lives in viewport-unit handling (`dvh`/`svh` instead of `vh`) plus forcing a layout recompute on load rather than waiting for first tap.

### iPad Undo Failure on Whole-Document Deletion **(Shipped, with known gaps)**

On iPad, Ctrl/CMD+Z fails to undo a deletion of the entire document. Needs investigation across three angles: whether the CodeMirror `history` extension's transaction/size limit is being hit by a single large delete, whether this is iOS-specific keyboard-event handling, or whether it also reproduces on desktop. Until reproduced cleanly, the root cause is open.

### iOS Predictive Text Disabled — `autocorrect: "off"`, `autocapitalize: "off"` **(Shipped v4.1.0, with backlash)**

#### What it does **(Shipped v4.1.0)**
...
#### Sean's pushback **(Shipped, with known gaps)**

Sean wanted **autocorrect and autocapitalize ON** on mobile, just NOT the iOS predictive-text bar:

> The update document detailed how *desperately* we need both of these on mobile and tablet, which you'll see in the notes below. We *also* want them *on* for the desktop app; in various settings (browsers, desktop app) the settings do not reflect actual settings.

> there nott autocorrectting at all still on movilw. weird. it wants. it doesn't underline mispellings. but quen i tap one i can change it. not that that is helpful unless it is an obvious misspelling. what we really need though is the autocorrwct.
...
### iOS Predictive Text Bar — The "Form-Field" Confusion **(Shipped, with known gaps)**
...
### Share Sheet — Web Share API + Custom Button **(Shipped v4.1.0)**
...
#### Known gaps **(Shipped, with known gaps)**

1. **Mobile share sends only URL, not content**:

> I just tried using the share button from mobile Safari and AirDrop to my computer and it just opened the IP URL and it doesn't have any of the content that I was typing.
...
### Touch Handlers — Cursor Placement & Highlighting **(Spec'd, not shipped)**
...
The transparent selection color is hardest to see on mobile, but it is not a mobile-specific issue — see § Editor Foundation § Editor Chrome Colors for the full call-out and spec.

### Cross-Window Sync on Mobile **(Cancelled — see v3.1.2 single persistent main draftpad)**

Sean's direct quote on the desired model: *Syncing content in new tabs of the same browser or new windows of the desktop app was the previous behavior which has no use-case I can think of. Do not sync dynamically.*
...
### Word, Character, Token Counter **(Spec'd, not shipped)**
...
#### Placement — Consider Native vs Web
...
### "Subtle Save Indicator" **(Spec'd, deferred)**
...
### Print to Simple PDF **(Spec'd, not shipped)**
...
**Caveat — Typora CSS theme reference**: the Typora CSS themes kept as a typography reference (`docs/archive/resources/typora-app-css-themes/`) may contain editor-highlighting CSS, not just export/PDF styling. Before adapting any of those rules into the print stylesheet, the files must be examined to separate the export-applicable rules from the editor-applicable ones — pulling them in wholesale would drag editor-surface styling into the print path.

**Reference point — Safari Reader mode**: opening Thot in Safari Reader mode renders the document cleanly — it strips the highlighting while keeping heading-size differentiation. That output is a useful reference for what the "clean prose" print/export mode should look like: not the highlighted editor view, but a plain document where headings are still visibly distinct.
...
**Status**: **(Spec'd, not shipped — spec locked; ready to promote to a BUILD packet)**. UX captured in Sean's narrative + 7 reference screenshots. The implementation spec was fully locked in a later planning round — the open questions that earlier framings carried have been decided (see Locked Decisions below). What remains is regenerating production code at BUILD time. Scheduled as v4.2.0.

### The Spec — Sean's Narrative (verbatim, full)

Sean's single-paragraph spec, reproduced in full because every clause matters:
...
- The stack is determined by the top-of-viewport position, not the cursor line (locked — see Q2 / Implementation Approach below).

### IDE-Standard Rationale (Why It's Table-Stakes)

> ### Why this matters
>
> Per Sean's framing: this is a must-have for moving developer-users away from their current IDE-based markdown editing, AND it surfaces a UX convenience that non-developer writers don't even know to ask for. Strong differentiator for both audiences.
...
A later planning round closed every open question this section once carried as Medium-confidence. These are decided, not directional. The earlier "needs a planning round" framing is obsolete.

- **Q1 — Render approach.** A pure DOM overlay, appended to the editor's outer DOM and positioned absolutely over the scroller's top edge. NOT the Panel API. NOT `Decoration.widget` — a widget would scroll with the content, which is exactly the wrong behavior for a pinned stack.
- **Q2 — Heading tree.** A memoized sorted array of `{ line, level, text, pos }` entries, invalidated only on `docChanged`. Per-scroll lookup against that array is a linear walk (or a binary search). NOT a per-frame `syntaxTree.iterate` — re-walking the tree every scroll frame is the performance trap to avoid.
- **Q3 — Smooth scroll, no jump.** When the next heading at level N approaches the pinned heading at level N, the pinned heading translates upward (`translateY`) by the scroll delta and slides off exactly as the new one takes its place. This is a continuous-scroll handoff — the outgoing heading is pushed out by the incoming one in lockstep with the scroll, the trick IDEs use. No discrete swap, no flicker.
- **Q4 — Line-wrapped heading.** If a heading wraps to more than one line, the overlay shows the first line only.
- **Q5 — Styling.** The stacked heading is styled identically to the in-document heading — same font, color `#FF9D00`, same weight. Document lines scroll *under* the overlay, so the overlay needs an opaque background (`colors.bg`, `#1a1a1a`) to keep the pinned headings readable as content passes beneath them.
- **Q6 — Performance.** 60fps on 100K+ line documents is non-negotiable. Profile on a synthetic 100K-line document before declaring the feature done. Escalation path if `updateStack` shows tasks over 16ms: first convert the linear walk to a binary search keyed by top line; then short-circuit the next-heading loop earlier; then debounce the update to `requestAnimationFrame`. If, after all three, it still cannot hold 60fps, the architecture is wrong — stop and surface the problem to Sean rather than shipping a janky stack.

### Implementation Approach

The feature is a `ViewPlugin` plus a DOM overlay. The plugin maintains the memoized heading tree (Q2), rebuilding it only when a transaction has `docChanged`. On scroll, it walks that array to find, for each heading level present, the most-recent heading whose position is at or above the top of the viewport, and renders one row per active level into the overlay container — H1 at the top, then H2, then H3, cascading down.

The handoff animation (Q3) is driven off the scroll delta: as the incoming same-level heading nears the pinned one, the pinned heading's `translateY` tracks the delta so it slides off precisely as the new heading arrives, a continuous motion rather than a swap. The overlay container sits over the scroller's top edge with an opaque `#1a1a1a` background (Q5) so document lines scrolling underneath stay hidden behind the pinned rows.

Production code is regenerated at BUILD time from these locked decisions; this section describes the approach, it does not pin the exact code.

### Locked Files

- **NEW `src/sticky-headings.ts`** — the `ViewPlugin` plus the overlay DOM management.
- **MODIFIED `src/editor.ts`** — one import, and one entry added to the extensions array, placed after `interactiveLinks()`.
- **MODIFIED `src/theme.ts`** — the overlay container styles (opaque background, heading-parity color/weight, positioning).
- **MODIFIED `src/click-handlers.ts`** — extended to handle clicks on overlay headings, scrolling to the first occurrence of that heading (reusing the heading-resolution logic shared with in-document anchor clicks).

### Visual Model

The intended look is "the page merges right into the toolbar" — the H1 sits flush at the top like a natural extension of the app chrome, the H2 below it, cascading down through the hierarchy. It should not read as a separate "pinned row" widget bolted on; it should read as the document's section path becoming part of the top of the app. That continuity is the intended effect, not just "a heading is pinned."

### Status Summary

- **Spec**: complete, clear, and locked (Sean's narrative + 7 screenshots + the Locked Decisions above).
- **Implementation**: approach locked; ready to promote to a BUILD packet — no further planning round needed.
...
- **Locked files**: NEW `src/sticky-headings.ts`; MODIFIED `src/editor.ts`, `src/theme.ts`, `src/click-handlers.ts` (see Locked Files above).
- **Inter-feature dependencies**: integrates cleanly with v4.1.0 URL clickability (shared anchor-resolution logic in `src/click-handlers.ts`); will need to be re-validated when the v5 scope-system lands.
...
### Strategic frame
  ...
  + **Why it's NOT optional** — *"Obviously a must have for a functioning app."*
  + **Spec lineage** — the original spec was drafted under the `v3.2.0` numbering and re-folded into v4 as `v4.1.0`.
...
### URL clickability — CMD/Ctrl+Click **(Shipped v4.1.0)**
  ...
  + **Why CMD/Ctrl gate, not bare click**
    - *"Optional UX choice: CMD+Click vs normal Click — Let's assume standard click for now, but maybe require CMD/Ctrl to avoid accidental clicks while editing"*
    - The BUILD locked the CMD/Ctrl gate: *"Only act on CMD/Ctrl+Click; bare clicks place the cursor as normal"*
  ...
  + **Cross-browser**
    ...
    - Confirmed working on Chrome (macOS `metaKey`) via Sean's preview environment. Safari `metaKey` and Firefox `ctrlKey` not separately verified — *"expected but not proven."*
...
### Local file path resolution — `/Users/...`, relative paths **(Spec'd, not shipped — deferred to native shell)**
  ...
  + **Original spec ambition**
    - *"Clicking a valid URL `https://...` or an absolute file path `/Users/...` opens it. If a URL, it opens in a new browser tab. If a local file path (and supported by environment), it will attempt to handle it gracefully (e.g., if we build out an Electron/Native shell later, but for the web app, it will primarily focus on `http`/`https` URLs)."*

  + **Status: deferred**
    - *"Local file paths are out of scope (deferred until/unless we ship a native shell that can resolve them)."*
    - *"Local file paths — out of scope for v4.1.0 (deferred until/unless we ship a native shell that can resolve them)."*
...
### Touch / long-press context menu — mobile equivalent **(Spec'd, not shipped — deferred to vNext)**
  ...
  + **Why it's needed**
    - *"iOS/touch has no concept of CMD+Click. Long-press → context menu with 'Open Link' is the canonical mobile pattern."*
  ...
  + **Status: deferred to vNext mobile UX bucket**
    - *"Out of scope for v4.1.0; capture in the vNext mobile UX bucket if not already there."*
    - *"Out of scope for v4.1.0; capture in a vNext mobile UX bucket."*
...
### `@noteName` cross-document links **(Concept, needs research)**
  ...
  + **Origin** — v1 future-evolution hook
    - *"v2 – Organization & Column View: Workspace as a folder of `.md` files. Finder-style column navigation. Configurable 'post-it' preview snippets per note."*
    - The `@noteName` mechanic was paired with `#ProjectTag` in v2 feedback: *"We'd use tags to somehow allow for connecting content as well so that certain drill-down spots would be 'auto' created based on the #ProjectTag and then when writing a note within a #ProjectTag you could @tag different notes based on their title."*
...
### `#ProjectTag` auto-sectioning **(Concept, needs research)**
  ...
  + **Origin** — v1 / v2 sketch
    ...
    - *"...certain drill-down spots would be 'auto' created based on the #ProjectTag..."*
...
### Cross-cutting note: where Interactive Markdown sits in the moat
  ...
  + URL clickability and anchor links are table-stakes — every IDE markdown editor has them; shipping them was about closing the "obviously a must have for a functioning app" gap, not differentiation.
...
The runtime highlighter is three cooperating mechanisms, each papering over a limit of the others. The full architecture is captured in `docs/THOT_APP.md` § *Architecture Explained* and was forensically rebuilt during v2.0.8.

**Three concerns:**

1. **`HighlightStyle.define()` in `src/theme.ts`** — the syntax-tag → CSS mapping. Generates single-class CSS rules in array order. **When the Lezer inherit mechanism places two CSS classes on the same `<span>`, the LATER CSS rule in the stylesheet wins**, so the array is ordered LOW priority first, HIGH priority last.
...
3. **A targeted `ViewPlugin` (`markerDecorations` in `src/editor.ts:55-129`)** — handles three context-dependent marker cases that Lezer's tag system cannot express because the default parser pre-tags markers as `tags.processingInstruction` at depth=0, and `combine()` merges base + extension rules placing depth=0 rules first (see `docs/THOT_APP.md`, and `@lezer/highlight/dist/index.js:246-263`). The three cases:
   ...
   - As of v4.0.0.4 a fourth case was added in the same plugin: `TaskMarker` containing `[x]` or `[X]` → darker checkbox color (decoration constant `checkboxCheckedDeco`). **Note**: v4.0.0 testing found this branch does not visibly apply — the inline-style decoration is out-specificity'd by the `tags.atom` HighlightStyle rule. Deferred to v5.

**Why depth-1 path matches beat depth-0:** The default markdown parser tags ALL markers as `tags.processingInstruction` at depth=0. A wildcard like `'Emphasis/...'` creates inherit rules on the parent node, NOT higher-specificity rules on the markers themselves. Path-based matches like `'Emphasis/EmphasisMark'` resolve at depth=1, which beats depth=0.

**Single source of truth**: `src/highlight-tags.ts` carries the entire colors object (~100 entries) plus the custom Lezer tag definitions. After v2.0.8 the prior `src/theme-reference.ts` was moved into the archive resources to enforce this.
...
| Custom tag          | Purpose                                                    |
| ------------------- | ---------------------------------------------------------- |
| `bulletMarkTag`     | Bullet list markers `-`, `*`, `+`                          |
| `orderedMarkTag`    | Ordered list markers `1.`, `2.`, `3.`                      |
| `bulletContentTag`  | Text content inside bullet list items                      |
| `orderedContentTag` | Text content inside ordered list items                     |
| `tableTag`          | Unified table coloring (pipes, headers, cells, delimiters) |
...
The palette comes from three nested layers of history. **CONFLICTS exist between layers**; the canonical current values are in `src/highlight-tags.ts`. Sean's reference TS file `docs/archive/resources/old-theme-ref.ts` was the original "accurate colors" reference.
...
| `fg` (foreground / default text) | `#e6e6e6` |
| `selection`                      | `#44475a` |
| `cursor`                         | `#e6e6e6` |
| `gutter` (line-number column)    | `#6272a4` |

#### Headings

| Role                              | Value              |
| --------------------------------- | ------------------ |
| `heading` (all 6 levels + marker) | `#FF9D00` (orange) |

Style: ExtraBold (800). Heading marker (`#`) and heading content render identical color and weight. Sean's explicit note: "I changed the headings so that they're the same size as the rest of the document. Please leave them this way."
...
| `strikethrough` | `#6272A4` (gray) + line-through |

**CONFLICT**: An older theme version lists italic at `#8aeefb` (cyan). The current `highlight-tags.ts` and `old-theme-ref.ts` both use magenta `#BF437F`. The cyan value appears to be a stale earlier value; the magenta is current.

Style notes: bold → ExtraBold (800); italic → ExtraBoldItalic (800i); strikethrough → Thin (100) with `text-decoration: line-through`.

#### Code

| Role                                                | Value                    |
| --------------------------------------------------- | ------------------------ |
| `inlineCode` (text + ` delimiters)                  | `#F34D3E` (red-orange)   |
| `fencedCodeDelimiter` (``` markers)                 | `#6767fc` (blue-purple)  |
| `codeBlockContent` (plain code-block text fallback) | `#8989e3` (light purple) |
| `codeLanguage` (language identifier after ```)      | `#F1FA8C` (yellow)       |

**CONFLICT**: An older theme version lists inlineCode at `#78de8c` (mint-green). Current canonical is `#F34D3E` red-orange — the older green is from an even earlier theme version. **Color-correction note from v2.0.8**: the prior `theme.ts` had `blockCodeDelimiter: '#8989e3'` which was actually the code CONTENT color; the fix mapped `tags.processingInstruction → #6767fc` so the fenced delimiter is now correctly blue-purple.

#### Links

| Role                                             | Value                                |
| ------------------------------------------------ | ------------------------------------ |
| `linkText` (`[text]`)                            | `#AB9DF2` (purple)                   |
| `linkUrl` (`(https://…)`)                        | `#8BE9FD` (cyan)                     |
| `linkTitle` (`"title"` in `[text](url "title")`) | `#AB9DF2` (purple, same as linkText) |
| `referenceLink` (`[ref]: url` definitions)       | `#50FA7B` (green)                    |

Style: linkText → Bold (700).

#### Images

| Role                       | Value                        |
| -------------------------- | ---------------------------- |
| `imageAltText` (`![alt]`)  | `#AB9DF2` (same as linkText) |
| `imageUrl` (`(image.jpg)`) | `#8BE9FD` (same as linkUrl)  |

Image and Link share the same Lezer node structure (both use `LinkMark`, `URL`). The colors object reserves separate keys for future customization but maps to identical values today; future differentiation would use a ViewPlugin checking the parent node type.

#### Lists

| Role                                  | Value                   | Notes                              |
| ------------------------------------- | ----------------------- | ---------------------------------- |
| `bulletMarker` (`-`, `*`, `+`)        | `#dfc532` (gold)        |                                    |
| `bulletContent` (bullet item text)    | `#8aeefb` (cyan)        |                                    |
| `numberedMarker` (`1.`, `2.`, `3.`)   | `#ff6b6b` (red)         |                                    |
| `numberedContent` (ordered item text) | `#f8a5c2` (pink)        |                                    |
| `checkbox` (`[ ]` open)               | `#8BE9FD` (cyan)        |                                    |
| `checkboxChecked` (`[x]` complete)    | `#5A7DB8` (darker blue) | Added v4.0.0.4. **Reads as "done"** |

**CONFLICT**: An older theme version listed bullet content at `#5feda4` (mint) and checkbox at `#50faad`. Current canonical values are the `#8aeefb` cyan / `#8BE9FD` cyan pair.

#### Block Elements

| Role                             | Value              |
| -------------------------------- | ------------------ |
| `blockquote` (`>` marker + text) | `#E6DB74` (yellow) |
| `horizontalRule` (`---`)         | `#93f9c6` (mint)   |
| `table` (pipes, headers, cells)  | `#e2ff79` (lime)   |

Style: blockquote → ThinItalic (100i). Horizontal rule needs `---` minimum (4 hyphens `----` don't render the full bar per Sean's screenshot review).

#### Special Syntax

| Role                                           | Value                  |
| ---------------------------------------------- | ---------------------- |
| `htmlTag` (`<div>`, `</span>`)                 | `#FF79C6` (pink)       |
| `htmlAttribute` (`class=`, `id=`)              | `#FFD866` (gold)       |
| `htmlAttrValue` (`"value"`)                    | `#F34D3E` (red-orange) |
| `comment` (`<!-- ... -->`)                     | `#6272A4` (gray)       |
| `escapeChar` (`\*`, `\[`)                      | `#FF79C6` (pink)       |
| `entity` (`&amp;`, `&lt;`)                     | `#FF79C6` (pink)       |
| `emoji` (`:smile:`)                            | `#FFB86C` (orange)     |
| `math` (`$inline$`, `$$block$$`)               | `#8BE9FD` (cyan)       |
| `footnote` (`[^1]`)                            | `#8BE9FD` (cyan)       |
| `superscript` (`^text^`)                       | `#8BE9FD` (cyan)       |
| `subscript` (`~text~`)                         | `#8BE9FD` (cyan)       |
| `hardBreak` (trailing-space or `\` line break) | `#6272A4` (gray)       |

Style: comment → ThinItalic (100i).

#### YAML Front Matter

Current shipped frontmatter color is whatever `colors.frontmatter` holds. The frontmatter mis-tagging bug, the inert `DocumentMeta` remap, and the SetextHeading2 root cause are documented in full below — see § Frontmatter / DocumentMeta Workaround (v4.0.0.3).
...
Organized into 8 sub-groups; these apply inside ``` ``` ``` blocks when `codeLanguages: languages` from `@codemirror/language-data` resolves a parser. Categories: keywords, names & identifiers, literals, operators, comments (all ThinItalic), punctuation & brackets, meta & annotations, HTML/JSX in code, errors. The full table is ~50 entries.

#### Future Markdown Extensions (slots pre-defined at `#e6e6e6` foreground until parser extension ships)

These show as foreground for now; require parser extensions not yet installed.
...
#### Task-List Item Text Coloring — Precise Rule

The `taskListText` slot above gestures at this but never states it. The rule the v5 scope system should enforce:

- A task-list item that begins with a **bullet marker** — `- [ ] text` — has its trailing text colored as **bullet content (cyan)**, because it *is* a bullet-list item that happens to carry a checkbox.
- If the task-list item is inside a **numbered list**, the trailing text is colored as **numbered content (pink)** for the same reason.
- The trailing text is NOT foreground white in either of those cases.
- A checkbox with **no preceding list marker** is the only case where the trailing text falls through to **foreground white**.

So the checkbox does not override the list-content color when a list marker is present; the surrounding list context wins, and the checkbox is treated as decoration on a list item.

### Marker ↔ Content Differentiation (Spec)

Sean's feedback lays out the requirement explicitly: every markdown element with a "marker" character and a "content" portion needs both labeled separately, even if they share the same color, so users can later target either independently.
...
Sean's original priority spec (verbatim, top to bottom = highest to lowest):
...
**Known nested-emphasis bug** (still unresolved): Italics in bold doesn't apply; bold in italics does. **Like *this* line** vs *Like **this** line* — both should apply ExtraBoldItalic (800i) but the bold rule currently overrides italic when bold is the outer wrapper. Filed as v5 work; the tree-nesting magic from Option A (inner span renders over outer span) is the canonical fix per `docs/research/1_DEEP/highlighter-architecture/OPTIONS.md`.

### Extended Font Weights for Emphasis

Sean's font weight system — 9 weights from the JetBrains Mono NL family, each mapped to a semantic role. Family stack: `'JetBrains Mono NL', 'JetBrains Mono', monospace`. Base font size: 12px (Sean explicitly fixed from initial 16pt in v2.0.0 Phase 7). Line height: 1.5 (raised from 1.0 in v2.1.4).

The weight ladder is what gives Thot its visual "weight rhythm": ExtraBold for the structurally important elements (headings, bold), Medium for body, Thin for "ignore me" content (comments, strikethrough), and ThinItalic for "soft voice" blockquotes. This is part of the trade dress moat — palette + typography pairing as defensible visual product.
...
The behavior Sean reported (screenshots `docs/archive/images/frontmatter-random-list-item-insert-{1..6}.jpg`): "in a numbered list, hit return for sub-bullet → delete auto-number → type `-` and space → the hyphen renders in the orange/bold frontmatter style."
...
**v4.0.0 runtime finding**: The remap is **inert** at runtime. `@lezer/markdown` does not emit `DocumentMeta` for markdown content at all — that tag only fires for HTML/XML `DoctypeDecl`. The real root cause of Sean's bug is **SetextHeading2**: a `---` line after a paragraph promotes the paragraph to H2 per CommonMark spec.
...
The v4.0.0.4 fix extends the existing `markerDecorations` ViewPlugin.
...
**v4.0.0 finding**: Decoration code is structurally correct but live testing showed `[x]` does NOT visibly render the darker `#5A7DB8`. Root cause: the `tags.atom` HighlightStyle rule cascades on top of the inline-style decoration. Two fix paths for v5:
1. Move to a `.thot-checkbox-checked` class technique (gains CSS specificity).
2. Wait for v5 scope system where this becomes a config knob (the recommended path — the highlighter research § 4 confirms inner-span-over-outer-span tree-nesting magic resolves this class of bug structurally).
...
**Fix**: restrict to direct list-item paragraphs:
...
This is the moat. The v5 work has two halves that ship together because they share infrastructure: the **proprietary scope/cascade rewrite** (here) and the **Intelligent Formatting dual-mode UI** (next H2 section). Both depend on `src/scopes.ts` — already drafted in the repo.

**Architecture decision (locked from research, `docs/research/1_DEEP/highlighter-architecture/OPTIONS.md` § 4):**
...
Which scopes are user-editable is gated by the `userCustomizable` flag — see § Preferences UI.
...
**Explicit design goal — portable standalone component**: the proprietary scope/highlight system should be designed so it could stand alone as a portable component usable in any text editor, not just Thot. Sean frames it as "pretty much a completely separate project." Practically, this means `src/scopes.ts` and `src/highlighter.ts` should not reach into Thot-specific globals or assume Thot's chrome — the scope table is data, the walker is generic, and the only Thot-specific binding is the color/typography palette it is handed. Keeping that boundary clean during the rewrite is a design constraint, not an afterthought.
...
Sean's original gut was "build our own — IP moat." The deep research closed this question explicitly. Verbatim summary from `OPTIONS.md` § 3.3:
...
**Recommendation locked: A + B2.** "Do A + B2 in that order. Skip C and D entirely."

### TextMate-Rule Heritage (from v1, informs v5 scope naming)

Even though we don't ship TextMate-based highlighting, the v1 work was done in VS Code/Cursor using a hand-tuned TextMate JSON file. The **scope naming convention** from that work is the informal precedent for the v5 scope vocabulary.
...
**Inheritance via scope nesting**: A rule for `markup.list.unnumbered.markdown` matches any token whose scope ends with that exact suffix; a more specific rule like `markup.list.numbered.markdown markup.list.unnumbered.markdown punctuation.definition.list.begin.markdown` (the "bullet inside numbered list" case) overrides because of longer-and-more-specific match. This is conceptually the same as Lezer's depth-1 path matching.
...
**Sean's original framing**: "After doing this, I finally think I understand TextMate and currently prefer it. When I use the 'Developer: Inspect Editor Tokens and Scopes' I can see the hierarchy and the one at the top is what highlights the tagged text." The TextMate inspector model — where a developer can click any character and see the cascading list of scopes resolved on it — is the gold-standard debugging affordance the v5 system should preserve.

### VSCode-Compatible Scope Highlighting Config **(Concept, needs research)**

Thot's semantic highlighting should be configurable the way VSCode and its forks (Cursor, Antigravity) do it — via `textMateRules` in a settings JSON. Sean's experience having to re-set up Antigravity's new separate IDE app made the pain concrete: in a VSC fork you just add your `textMateRules` to the settings JSON and you're done. Thot losing that workflow would be a real friction point for any developer carrying their highlighting config over.

What this implies for the v5/v6 design:

  + **Support pasting raw VSCode `textMateRules` JSON directly into Thot.** Developers will very likely want to drop their existing VSC `textMateRules` straight into Thot rather than re-pick every color. Support that path.
  + **If raw paste proves unreliable, provide an import path** — and therefore an export path too. Sean's earliest attempt at the project handed in his saved `textMateRules` and the scopes didn't highlight correctly; if pasting the same JSON a VSC fork uses turns out to be flaky, an explicit import/export round-trip is the fallback.
  + **Support JSONC (JSON-with-comments).** Sean keeps a commented copy of his rules — comments label what each scope is for — so the parser for whatever paste/import path ships must accept JSONC, not strict JSON only.
  + **The open question to research**: what would Thot's scope system need in order to read and apply VSCode `textMateRules` JSON *exactly as the VSC forks do* — so the same rules produce the same colors? This is the core unknown, and it ties into the v5 scope-system rewrite (the dot-separated TextMate scope vocabulary is already the precedent for `src/scopes.ts`, per the heritage section above).
  + → Research: `docs/research/1_DEEP/highlighter-architecture/OPTIONS.md`

This JSON path is for developers. It coexists with — does not replace — the friendly visual scope-color UI for non-developers (see § Preferences UI); both surfaces drive the same underlying scope config. Importing VSC-fork settings more broadly (beyond just highlighting) is covered in § File Management & I/O § IDE feature parity.

### Highlighting "Themes" (Presets) **(Concept, needs research)**

Highlighting "Themes" are named scope-color sets — presets a user can pick from and switch between. Full framing (the terminology, the curated-vs-user-saved split, the must-expose color distinctions) lives in § Preferences UI § Project themes / theme presets.
...
**Status**: **(Research locked; UX scaffold awaiting Sean's fill-in)**. The mechanics are spec'd in the research. The user-experience layer — what a non-markdown writer actually sees, how mode switching feels gesturally, what auto-detections feel right vs surprising — exists as a 9-prompt scaffold (`docs/research/1_DEEP/feature-research/FORMATTING_UX.md`) where Sean's brain-dump is captured before synthesis. Sean's original detailed UX pitch is **"lost"** — as he put it: "It would be great if we could find the actual user experience written description of how the interface worked... That was very well pitched... Perhaps you can find it."
...
The pitch in two parts — the public framing, and the design intent (see § "Don't Block Out Non-Markdown Writers").
...
**Cognitive-load framing** (Sean's voice): "I have trouble reading big documents, but when types of chunks of text are visually identified by color it somehow creates a cognitive load easing which is what all good design should be trying to do."

**Two user segments served simultaneously**:
...
The `src/scopes.ts` already-drafted `auto.*` group represents the "intelligent" half of intelligent formatting: rules that fire on patterns in plain text, with no markdown notation required. Per the scaffold prompt in `FORMATTING_UX.md` (verbatim):
...
**What auto-behaviors would be WRONG (negative space)**: open question awaiting Sean's fill-in. Examples that come to mind for the eventual answer: a user typing in all caps as a stylistic choice and not wanting the editor to "interpret" it; a user using parentheticals as core content not asides; questions used as section headers that shouldn't be italicized.

### Notation Hiding via `Decoration.replace()` + Floating Context Menu

**Visual mode mechanism** (verbatim):
...
**Floating context menu mechanism** (verbatim):
...
Sean has not chosen. The framing (verbatim):
...
**Strategic recommendation in v3**: "Start with Dual Mode (v3.0.0), add Hybrid Mode later (v3.1.0) if demand exists."

**The v5 stance** (verbatim):
...
**Open questions for the decision** (`FORMATTING_UX.md`, scaffold prompts awaiting Sean's narrative):
...
**Explicit-vs-auto-detect interaction** (`FORMATTING_UX.md`):
...
Sean's original phrasing (verbatim):
...
**The terminology shift**: by v4.0/v5 framing this is no longer "RTF mode" — since users "barely know what TXT is," the explicit mode label is **not** "RTF." It's just "Visual" or no label at all (users who don't know markdown never see the distinction).

**The UI surface that survives**: the floating/contextual formatting toolbar with **prominent keyboard shortcuts written on the buttons** (verbatim):

> The menu will contain buttons for common tools, highly prioritizing the display of keyboard shortcuts (e.g., `⌘B` for bold).

**The four UI variants explored** (verbatim):
...
**Anticipated keyboard shortcuts** (from an early SwiftUI menu sketch):
...
### Selection Auto-Wrap (relates to formatting UI)

Selection-then-delimiter wrapping (highlight a word, type `(` or `"` and it wraps; the markdown-marker `*`/`~`/etc. variant is spec'd but not shipped) is specified in full in § Editor Foundation § Paired Delimiters / § Shift-Marker Wrapping.

The angle that matters *here*: this behavior is **already a step toward "format without knowing markdown"** — a user who selects "important word" and types `*` gets `*important word*` without ever having known the asterisk was the bold/italic syntax. The future formatting toolbar layers on top of this with explicit B/I/etc. buttons.
...
**Status**: vNext concept (AI integration phase), not v5. Tab autocompletion is part of the `@claude` AI integration milestone.
...
**Why this matters for Intelligent Formatting**: the auto-detection rules in `auto.*` are themselves customizable — they are gated by the `userCustomizable` flag (see § Preferences UI). A natural language layer on top of that gives users non-technical access to: "turn off the question-mark italic thing", "make ALL CAPS green instead of bold-yellow", "stop dimming my parentheticals". The v1 concept folds in cleanly.
...
- `docs/research/_superseded/feature-research/FORMATTING.md` — original mechanics spec (Decoration.replace, showTooltip facet, context menu); superseded, kept for the specific mechanics reference.
...
### Strategic frame
  ...
  + **Why preferences are central to the moat**
    ...
    - *"This frankly I would love. The user could even create 'project themes' or maybe different sections of the column view drill-down would be give different scope highlight colors so that it is immediately recognizable if you're in the right section of your notepad"*
  ...
  + **One-place-per-preference principle**
    - Any given preference has exactly ONE place it is configured — no duplicate dials, no settings that live in two surfaces and drift apart.
    - Where the operating system already owns a setting, Thot defers to the OS rather than re-implementing it in-app. This is especially load-bearing for the native-wrapper preference mapping: a native preference that the platform owns (e.g. system spellcheck, system text size, biometric policy) should map to the OS surface, not get duplicated inside Thot's preferences modal.
    - This keeps the preference surface honest as it grows and keeps the web/native "drop-in" boundary clean.

  + **Housekeeping — icon-asset consolidation**
    - The project currently has duplicate icon assets scattered across roughly four directories, plus a missing `public/manifest.json`.
    - A cleanup pass should consolidate the icons into one canonical location and fix the favicon and manifest references so there is a single source of truth for app iconography.
    - → Research: `docs/research/1_DEEP/feature-research/ICONS.md` (a consolidation-plan research doc).
...
### Preferences modal — sections
  ...
  + **Editor behavior** — boolean and numeric toggles.
...
### Shared Live-Preview Window

The preferences UI is built around a single, shared live-preview window — one viewer that demonstrates the effect of whatever setting the user is currently adjusting. UI reference screenshots: `docs/archive/images/ui-example-settings-tab-from-textedit-app-01.jpg` and `-02.jpg` (the macOS TextEdit app's settings tabs). Those are macOS-native examples; the web-app preview should be designed so it translates cleanly to the native preference surfaces when the build is dropped into the wrapper (see the standing constraint in § Strategic Pillars — every setting's native-translation path is part of its spec).

  + **One preview, reused across tabs.** The highlighting settings tab shows a live preview of highlighted example text; clicking a scope updates *what the preview demonstrates*. The same preview window is reused by the other settings tabs — bold, italics, font, character spacing / tracking, etc. — each of those settings lives on its own tab, but they all share the one preview window. As the user switches tabs, the preview window stays put and changes its content to match the active tab.
  + **Per-scope row layout (highlighting tab).** For each highlight scope, the UI row shows three things together:
    1. A **plain-language describer** of which part of the text the highlight applies to (e.g. "bullet marker," "the text after a bullet") — written for non-developers.
    2. The **technical scope name** in faded, smaller text next to the describer (e.g. the `bulletMarker` / TextMate-style scope name) — there for developers without dominating the row.
    3. The **visual preview** of that scope (in the shared preview window when the row is selected).
  + **The friendly visual UI is mandatory.** This visual scope-color UI is the path a non-developer uses — pickers, describers, the live preview — so they can set scope colors without ever touching JSON. It coexists with the developer-facing VSCode `textMateRules` paste/import path described in § Semantic Highlighting & Theme; both surfaces must drive the same underlying scope config.
  + **Preview window as a context surface, not just a sample.** The shared preview/context window can extend beyond the highlighting and typography tabs to *any* settings tab, to give novice users helpful context about what a setting does. The example Sean gives: if Thot ever exposes Git / SSH configuration the way the VSC forks do, the context window would explain — in plain language — what that setting is actually for, so a non-technical user isn't staring at a setting they don't understand. The window's job is "show or explain what this setting means," whether that's a visual sample or a plain-language note.
...
The following is the aggregated list of every "this should be a setting" mention across the corpus. Each item should be a row in the preferences UI's Editor Behavior section unless flagged otherwise.
  ...
  + **Line numbers visibility**
    ...
    - Width is already tokenized as CSS custom property `--thot-line-number-width` (default `16px`) for future width-adjust control.

  + **Line number column width**
    - Numeric value `--thot-line-number-width`. The v4 polish chunk tokenized this specifically *so the v6 preferences UI can adjust it.*
    - The adaptive-spacing behavior (1–3 digits = static width, 4+ digits nudges wider) described in § Line Number Column & Gutter is the runtime behavior; the preference governs the *base* width for 1–3 digit case.

  + **Font size**
    - Numeric. Sean's feedback records 16pt being "HUGE" and wanting it set reliably to 12pt; the inconsistency he encountered while trying to change it himself is a strong argument for putting this in a UI.
    - *"I went to change it back to 12pt, ran `npm run preview`, tried incognito, tried hard refresh and nothing — SO — it is still all TINY and I can't get it to change"*

  + **Line height**
    - Numeric (line-height multiplier).

  + **Max line width**
    - A cap on line length (e.g. 80ch / 100ch / unlimited) that, on wide screens, centers the content column rather than letting text run the full viewport width.
    - 12pt text crossing a very wide monitor feels strange to read; a width cap restores a comfortable measure without forcing the user to resize the window.
    - Numeric (or preset) value; `unlimited` keeps today's full-width behavior as one option.
  ...
  + **Theme switcher — light / dark / system / custom**
    - Currently dark-only. Light mode is mentioned as a future direction: *"Light view"*
  ...
  + **Spellcheck toggle**
    ...
    - Per `docs/THOT_APP.md`: *"Enabled browser spellcheck via `spellcheck: 'true'`"*
  ...
  + **Counter visibility (words, characters, tokens)**
  ...
  + **Customizable highlight colors per scope**
    - This is the core Preferences feature — see Theme section above.

  + **Project themes / theme presets** **(Concept, needs research)**
    ...
    - **Terminology**: when the project says "themes," it means *highlighting presets* — named sets of scope-to-color assignments the user picks from and switches between — **not** app-chrome / light-vs-dark themes. This is what earlier mentions of "themes" in the project always meant. App-chrome theming is the separate concern handled by the light/dark/system theme switcher below.
    - Thot offers both *curated* preset Themes (shipped, named scope-color sets) and *user-saved* presets the user names themselves.
    - Two color distinctions these Themes (and the per-scope settings behind them) must expose, because they are visually important to Sean:
      - The **bullet-marker vs bullet-content** distinction — the `-`/`*`/`+` marker and the list item's text are separately colorable (already true in the shipped palette; themes and the settings UI must keep this controllable, not collapse it).
      - The **plain-text vs list-text** distinction — plain paragraph text and list-item text are different colors and the user can control each.
    - Per-folder / per-`#ProjectTag` theme association — entering Project X automatically loads its theme. Depends on the columns workspace shipping first.
    - Theme presets, the friendly visual scope-color UI (above), and the VSCode `textMateRules` paste/import path (see § Semantic Highlighting & Theme) are three faces of the same customization surface.
  ...
  + **Biometric lock per-note** (vNext, native-only)
  ...
  + **Reminders integration trigger syntax toggle** (vNext, native-only)
  ...
  + **Heading stacking visibility** (v4.2.0+)
    - Toggle the sticky-heading overlay on/off.
    - Configurable max-depth (only show H1+H2, vs all six levels).

  + **Line edit indicator colors**
  ...
  + **Heading font sizes**
    ...
    - Headings being same-size-as-body is a current intentional design choice (per `docs/THOT_APP.md`). A power-user might want larger headings; preference setting.

  + **Print / Export to PDF settings**
  ...
  + **Default new-document name format**
    - Controls how a freshly created document is named when the user hasn't given it one.
    - Default format: the current date plus the 24-hour `HH:MM` time.
    - A preference so users who prefer a different convention (date-only, sequential numbering, custom prefix) can set it once.

  + **New-document templates**
    - A space where the user picks a template that is applied to every new document.
    - The default template inserts a single H1 containing the date and time — so a new doc opens with a dated heading rather than a blank line.
    - Pairs naturally with the default new-document name format above; a user could keep both, one, or neither.

  + **Tabbed-document view (PWA)**
    - A configurable default for whether the PWA's tabbed-document view is on. The feature itself (multiple documents as tabs within one window) is specified in § Persistence & State § Tabbed-document view.

  + **Line numbers default — open note**
    - Line numbers currently ship default-ON. The original v1 intent was default-OFF.
    - Flag the default as revisitable: the toggle itself is settled (see "Line numbers visibility" above); what's open is which way the default should point out of the box.
...
### AI-first design philosophy
  ...
  + **The tension this captures**
    ...
    - Per the AI Features research bucket: *"Sean's instinct: the goal is *ambient* AI that reduces cognitive load, not *foreground* AI that interrupts thought."*
...
### Predictive tab completion (Cursor / Anti-Gravity reference) **(Concept, needs research)**
  ...
  + **The vision**
    ...
    - *"The amazing AI integration for tab completion, autocompletion, and auto-formatting that occurs in some markdown editors is still something that non-IDE users have very little experience with."*
    - *"People who use Google Docs have no idea that they can format a 10 page document with so many styles in just minutes — they have NO IDEA that is even a thing yet — how do we capitalize since idk why GoogleDocs hasn't when their markdown Anti-gravity one works great"*
  ...
  + **Architecture hooks in CodeMirror**
    - Per the AI Features bucket: *"Thot's editor is CodeMirror 6 — predictive completion has reasonable hooks via `ViewPlugin` + autocomplete extensions."*
  ...
  + **Open research questions from the bucket**
    - *"Predictive tab completion in normal prose — Cursor/Anti-Gravity-style. What's the user experience when this is good vs. bad? Cost per completion? Latency budget? How is it disabled gracefully?"*
    - *"Latency budget for predictive features — at what response time does a tab-completion stop feeling helpful and start feeling laggy? (Reference: Cursor's threshold is ~200ms.)"*
  ...
  + **Inline predictive arithmetic completion** **(Speculative — future watching)**
    - A narrow, non-AI flavor of predictive completion: typing an arithmetic expression that ends in `=` suggests the result inline — `10+12=` offers `22`, the way macOS TextEdit does.
    - This needs no model call; it's a deterministic local evaluation surfaced through the same ghost-text mechanism predictive completion uses.
    - Worth keeping on the radar as a small delight that fits the "format/compute without leaving the writing surface" framing.

### Format-on-save (Anti-Gravity reference) **(Concept, needs research)**

  + **Origin**
    - *"Anti-Gravity's chart cleanup is the canonical example: messy ASCII tree → perfect tree on save."*
    - *"Auto-Formatting Scopes — Example: Recipes — You paste in the basics of a recipe and BOOM it cleans it up all pretty for you — Bonus points if we figure out where they'd want to export them to then — General word processor formatting that is like Cursor/Anti-Gravity's Tab model"*
    - *"Subtle AI integrations — I'm also really into the idea of even more subtle things like automatic formatting"*

  + **What gets formatted on save**
    - From the AI Features bucket: *"What other 'tidy this up for me' actions are valuable in a markdown editor? (Heading hierarchy normalization, list nesting cleanup, table column alignment, link reference de-duplication.)"*
    - Concrete candidates:
  ...
  + **CMD+S as the chart-perfecting trigger** **(Concept, needs research)**
    - Format-on-save above is generic about "on save." Sean's specific want is concrete: **CMD+S** should be the trigger that auto-perfects a messy hand-made chart or diagram in the document. This is the Anti-Gravity behavior he wants replicated directly — in Anti-Gravity, a messy ASCII chart/tree "perfects" itself the moment you save.
    - The chart-cleanup behavior is therefore tied to the CMD+S save action specifically, not to the invisible autosave debounce. Pressing CMD+S already force-flushes the localStorage queue (see § Persistence & State); the chart-perfecting pass would ride on that same explicit-save keystroke so the cleanup is deliberate and predictable, exactly the way Anti-Gravity times it.
...
### Model choice (local vs hosted) **(Open — needs research)**
  ...
  + **Options matrix from the bucket**
    - *"Predictive completion provider/model choice — local model (small, free, private) vs. hosted (better, costs $/token, requires server). What are 2026 options at each end? (Claude Haiku, GPT-4o-mini, Llama 3, Gemini Nano on-device.)"*

  + **Architectural axis**
    - *"Where does AI live architecturally?"*
  ...
  + **Sean's reference points**
    ...
    - *"Stripe billing has LLM token that adjusts for model"* — relevant for the paid-tier hosted-AI billing surface; see Monetization section.
...
### Cost / Stripe AI billing coupling **(Open — needs research)**
  ...
  + **Why it's coupled to monetization**
    - From the AI Features bucket: *"Cost model directly affects `monetization` bucket — paid tier likely centers on AI affordances."*
    - From the Monetization bucket: *"Stripe AI billing for token usage — Sean noted Stripe has tools now for charging-or-being-charged for AI tokens. Investigate: how does this work in practice? Does it support per-user metering and a sane UX?"*
  ...
  + **Free tier constraint** (links to Monetization section)
    - *"Sean's instinct: the free tier must remain genuinely useful (not a 3-day trial with paywalls); the paid tier earns its money by unlocking real value (sync, collab, AI affordances)."*
...
This is one of Sean's earliest and most-returned-to ideas. Surface it deeply.
  ...
  + **v1 origin (verbatim)**
    - *"v3 – Declarative Customization via AI: Config files (`PreferencesModel` / JSON) that define: Layout (columns, panes). Editor preferences (font, theme, behaviors). Note metadata (tags, pinned/priority). An AI layer that translates natural language → config changes."*
  ...
  + **v4 cross-reference**
    - The v4 features doc keeps the bullet alive: *"+ Declarative Customization via AI"*
  ...
  + **What it does — the "Make headings blue" loop**
    ...
    - Live preview shows the change immediately — *"Live preview"*.
  ...
  + **Schema requirements**
    ...
    - AI-editability is gated by the `userCustomizable` flag — the same flag that gates UI exposure (see § Preferences UI).
...
The three subsections below are long-horizon ideas kept on the radar — visually separated here from the locked AI philosophy above. None is a v5 or v6 commitment.

#### Specialized AI assistant "scopes" / personas **(Speculative — future watching)**

A long-horizon direction: specialized AI assistant "scopes" or personas the user can engage for a particular kind of writing, each tuned to a workflow rather than being a generic chat. Sketched examples:

  + **Memoir / Journal Buddy** — journaling prompts, memory organization, timeline generation from dated entries.
  + **SEO Buddy** — headline → keyword suggestions, image → caption, caption → hashtags.
  + **Meditation Buddy** — guided journaling, mood tracking over time.
  + **Completion Tasks** — content-type auto-formatting for recognizable document kinds (recipes, meeting notes), where the assistant shapes raw input into the expected structure.

**Tension to acknowledge**: the section's existing "no ghostwriting tools" stance may put some of these deliberately out of scope. A persona that drafts prose for the user edges into ghostwriting; one that organizes, prompts, formats, or extracts is a writing-flow affordance. These personas are kept on the radar, but each would need to be checked against the no-ghostwriting line before it became a real feature — some are likely out of scope by design.

#### Proprietary ML models for formatting / highlighting decisions **(Speculative — future watching)**

A long-horizon IP idea: train Thot-proprietary ML models to manage the intelligent-formatting and semantic-highlighting decision systems — the systems that decide *what* gets formatted or highlighted *how*. This would be a defensible moat layer beyond the parser (a commodity, per the highlighter research) and beyond the curated wordlists. The parser is Lezer's; the curation is editorial; a trained model that makes the formatting/highlighting calls would be a third, harder-to-copy layer. Cross-referenced in § Intelligent Formatting and § Semantic Highlighting, where the decision systems themselves live. Far-horizon — not v5 or v6.

#### Direct popular-feature integration — skipping the automation step **(Speculative — future watching)**

A speculative direction: rather than only offering automation hooks, Thot could offer popular features directly — "jump over the automation step and offer popular features directly." Examples floated: social-media post scheduling, grocery-list management, content-type auto-formatting and export to platforms. The framing is that for some workflows the user doesn't want to wire up an automation; they want the feature itself. Kept on the radar; would need to be reconciled with the focused "writing-flow affordances" scope before any of it became real.

### What NOT to build

  + **From the AI Features bucket (verbatim)**
    - *"What NOT to build — chat sidebars, ghostwriting tools, image generation. Stay focused on writing-flow affordances."*
  ...
  + **Expanded**
    ...
    - **No "AI everywhere" feature creep** — the AI-first design philosophy explicitly resists this: *"Always ask: Can we do this without AI? Resist temptation to put AI in everything."*
...
### Market thesis — bring IDE AI tools to mainstream non-markdown users
  ...
  + **The migration target**
    ...
    - *"Beyond developers, Thot's aim is to bring the magic of modern IDE markdown editors, like syntax highlighting and tab auto-completion, to users who default to Apple Notes or Google Docs; the kind of users who aren't likely to be persuaded to jump into learning markdown."*
    - *"No matter how you write, Thot's interface instantly and intuitively adapts. Many of these users have no idea that you can format an entire 20 page document in minutes thanks to tab completion, which is frankly still magical."*
  ...
  + **Market signal — pre-formatted output is already shipping**
    - A concrete signal underscoring the urgency of Thot's AI-formatting work: meeting-recorder devices are already advertised as delivering notes with the formatting *already done* before the user even opens them. The user records a meeting and the output arrives structured — headings, bullets, action items — with no cleanup step.
    - This means competitors already ship pre-formatted output as a baseline expectation, not a premium feature. It is part of the "open market, no-brainer" rationale for Thot's AI features: auto-formatting is no longer a delighter, it is fast becoming table-stakes, and Thot should treat it as such rather than a late-stage add.

  + **AI affordances for the wider audience**
    - From the AI Features bucket: *"AI affordances for the wider audience (non-dev markdown writers) — what AI feature would matter to a Google Docs migrant? Probably *not* 'complete my code.' Maybe 'rewrite this paragraph in active voice,' 'summarize this section,' 'find the heading I'm looking for.'"*

  + **Apple Intelligence overlap**
    - iOS 18+ ships Writing Tools (proofread, rewrite, summarize, refine) system-wide. *"Apple Intelligence-powered writing features — Proofread, refine, rewrite, summarize are available in PWA with right click — But iOS 18+ Writing Tools supposedly are only available native"*
...
### Tab Groups auto-naming reference (Dia browser)
  ...
  + **Dia also auto-*groups*, not only auto-names**
    - Beyond auto-naming a Tab Group, Dia also auto-*groups* related pages — it clusters tabs by similarity when links are opened from within a page, building the group itself rather than only labeling a group the user already made.
    - Translation to Thot: the smart behavior isn't just titling, it's clustering — when notes are spawned or linked from within another note, Thot could auto-group the related set into a workspace section, then auto-name it. The grouping half is the harder, more interesting half.
...
### The "single desk pad → AI organizes into a notebook" mental model

The mental model that ties AI organization to the multi-note workspace: the user dumps everything into one freeform pad — the single desk pad, no structure required up front — and AI is the bridge that sorts that dump into the structured multi-note workspace. The user never has to organize as they write; they write freely, and the AI is what turns the pad into a notebook with sections. This keeps the draftpad-first promise intact (no forced organization, see § Workspace & Multi-Note's Notion-skepticism note) while still delivering structure — the structuring step is the AI's job, not the user's.
...
### Tabbed-document view (PWA) **(Concept, needs research)**

A tabbed-document view for the PWA: multiple documents open as tabs *within a single window*, rather than each living in its own browser window. This is distinct from the existing separate-window CMD+N model — CMD+N spawns a whole new window/tab at `?id=temp-<random>`; the tabbed view keeps several documents accessible inside one window through an in-app tab strip.

The two models can coexist: CMD+N for a genuinely separate window, tabs for several docs in one place. Whether the tabbed view is on by default is a preference (see § Preferences UI, "Tabbed-document view (PWA)" toggle). Each tab still needs its own persistence partition, so the `windowId` partitioning primitive (`thot:content:<id>`) extends naturally to per-tab content keys; the open work is the tab-strip UI and how tabs are restored on cold launch given the Safari URL-stripping constraint documented below.
...
#### What was planned (v3.1.1)
  ...
  + **Shipped at**: commit `fb425e9`, tagged `v3.1.1-multi-window`
...
#### What went wrong on macOS Safari
  ...
  + Effectively: **URL state does not survive a Safari standalone PWA cold launch.** Sessions in `localStorage` (or `IndexedDB`) survive; URL state does not necessarily.
...
Per Sean's request: "Instead of trying to orchestrate an inherently incompatible multi-window environment within Safari's PWA constraints, we are pivoting to a single-draftpad approach."
...
#### Why this is the right call (user-decision rationale)
  ...
  + The v3.1.1 multi-window machinery is **still in the code** — it wasn't ripped out, it was repurposed. The `setPersistenceId(id)` / `setStateId(id)` partition primitives now support the `main` default + `temp-*` window pattern.
  + The "single permanent draftpad, plus disposable scratch windows" model **matches the original v1 mental model** anyway: "Thot v1.0.0_deskpad is not a knowledge management system, it's a single legal notepad on a desk". The persistence pivot accidentally restored the v1 intent.
...
#### Open thread (deferred from v3.4.0 planning, never implemented)
  ...
  + The v3.4.0 planning round proposed an additional layer: **maintaining a `thot:open-windows` registry in `localStorage` listing every active `?id=*` window. On app launch with no `?id=` URL parameter, re-open the registry's entries (the main pad + each surviving temp window) as separate PWA windows / tabs.** Macros around macOS Safari's PWA launch-param stripping (the v3.1.1 lesson) would apply: keep the registry as the source of truth, not URLs.
...
### Service Worker / PWA cache (relevant to persistence UX)
  ...
  + **Aggressive caching gotcha**: changed a source file but `npm run preview` shows the old version? You forgot to `npm run build` first, OR the service worker cached the old version — incognito or `Cmd+Shift+R`.
  + **No PWA install promotion**: per Sean's locked stance, the PWA install button/banner should be suppressed. Real native shells come in vNext; PWA muddies the upgrade narrative.
...
### Open Questions — Persistence & State
  ...
  + **localStorage quota at scale**: a single 5–10MB origin quota across all `temp-*` partitions could become a problem if temp windows accumulate. **Locked decision (prior planning round)**: temp-window IDs are durable and do NOT auto-expire — there is no N-day expiry. A temp window's content persists until the user explicitly clears it. If quota pressure ever becomes real, the answer is a user-facing "you have N old temp windows, clean up?" affordance, not silent time-based expiry.
...
### File-operation terminology — "Open" / "Save As", not "Import" / "Export" **(Shipped v3.1.0, with refinements)**
...
### Current shipping behavior **(Shipped, with known gaps)**
  ...
  + **Auto-detect document title from the first `# Heading`** **(Concept)**
    - An untitled / unsaved document should derive its display title — the title bar, the browser tab title, share prompts — deterministically from its first `# Heading`. Type `# Q4 Planning` at the top of an unsaved pad and the window/tab reads `Q4 Planning` without the user naming the file.
    - This is the **simple, non-AI** version of titling: a pure first-heading lookup with no model call, deterministic and instant. It is distinct from the **AI-driven auto-titling** already described in § AI Integration (the Dia "Tab Groups auto-naming" pattern — auto-titling an untitled doc from its first few lines of content via an LLM call). Both mechanisms are real and should be acknowledged as separate: the first-heading derivation is the cheap deterministic baseline; the AI auto-titling is the smarter fallback for docs with no heading or with content the heading doesn't summarize well.
...
### Known gaps and testing results
  ...
  + **AirDrop / Share from iOS Safari sends only the URL, not the content.** Tested on iOS: pressing Share → AirDrop just opened the IP URL on the recipient computer with no content. Same behavior for email. Same in Chrome on mobile.
  + **Share button works on desktop localhost, not on mobile IP preview.** `http://localhost:4173` shows the share button and shares content; `http://192.168.0.36:5173` (the mobile testing IP) doesn't display the custom share button at all. On mobile, Safari's native share button takes over and sends only the URL.
  + **Mobile cursor and selection are very hard to use.** "Every tap it responds by either entering into edit text or exiting the text editing stance." The only way to paste was to type random letters, double-tap to highlight, then use the paste option from the highlight menu.
  + **Highlighted text color (light green on charcoal) is very hard to see on mobile.** See § Editor Foundation § Editor Chrome Colors.
...
These come from v2 forward-looking sections. They have not been formally re-scoped for v4+ but are captured here so they don't fall out of the picture.
  ...
  + **Export to simple PDF / Print** **(Concept, needs research)**
    - Both originate as v2 forward items (#7 export-to-PDF, #8 print). Specified in full in § Counters & Productivity § Print to Simple PDF — the two print modes, the print-only CSS approach, and the Typora / Safari-Reader references.
  ...
  + **Export-to-Notion bridge** **(Speculative — future watching)**
    - A candidate integration: an export-to-Notion bridge via the Notion API. Thot would be used as the freeform "pre-planning" draftpad — the messy first-thoughts space — with a path to push organized content into Notion when the user is ready, because "taking notes in Notion has no draftpad vibe."
    - **Tension to acknowledge**: this document elsewhere uses Notion as negative-space — a definition of what Thot is *not* (no databases, no nested pages, no block-based content model; see § Intelligent Formatting and § Workspace & Multi-Note). So this is a speculative *integration*, not a core direction — Thot does not become Notion-like, it interoperates with Notion. The reason it stays on the radar at all is that the Notion API is genuinely good, which makes the draftpad-to-Notion handoff a clean enough path to keep watching rather than dismiss.

  + **Share / AirDrop — choose titled content vs blank notepad** **(Concept, low priority)**
    - When a document has a `# Title` heading, a share / AirDrop action should prompt the user to choose between sharing the titled content versus sharing a blank notepad — so the user explicitly controls what gets sent rather than always sending the current document.
    - Low priority; most relevant once multi-note support exists and "which document am I sharing" becomes a real question.

  + **Share-link behavior — relabel, don't conflate** **(Design insight)**
    - The current share-link behavior — a recipient who opens the link gets a fresh COPY of the text in a new app instance — is actually a desirable capability. The problem is that it is *mislabeled* as "sharing."
    - True "sharing" should open the shared document *itself* (the same document, not a copy). The copy-into-a-new-instance behavior is worth keeping, but as a distinct feature with its own name/position — not called "sharing."
    - So: keep the copy-to-new-instance feature, rename and reposition it; build real shared-document sharing separately. (Real shared-document sharing intersects Real-Time Collaboration's read-only-by-default share model — see § Real-Time Collaboration.)

### IDE feature parity — items that win IDE users

These three items come directly from the IDE-for-Markdown Replacement pillar. They are the kind of at-a-glance, low-friction conveniences that VS Code / Cursor / Antigravity users take for granted and would expect from a tool pitched as an IDE replacement for markdown work.

  + **File-explorer git-status indicators** **(Concept, needs research)**
    - In an IDE's file explorer, committed files render plain while modified-but-uncommitted files are marked — a color, or an "M" next to the filename, etc. — so the user can tell at a glance which files have unsaved-to-git changes.
    - Once Thot has a multi-note workspace / file-explorer surface, it should carry the same kind of at-a-glance state indication for files: a clear, low-noise marker distinguishing committed/clean files from modified ones.
    - This is the file-explorer analogue of the line-number edit/add indicators already spec'd in § Line Number Column & Gutter — the same "show me what changed" instinct, at the file level instead of the line level.
    - → Research: `docs/research/1_DEEP/ide-feature-parity/1_REVIEW.md`

  + **Importing settings from VS Code / Cursor / VSC forks** **(Concept, needs research)**
    - VSC-based IDEs (VS Code, Cursor, Antigravity) let a user import settings from another fork on first setup. They expose this through a verbose four-way split — "Settings Default UI," "Settings Default JSON," "Settings User UI," "Settings User JSON."
    - Thot's design will *not* need that verbose split — its preferences surface is simpler by design (see § Preferences UI). But Thot *should* support importing a VSC fork's settings, so a developer migrating from VS Code / Cursor / Antigravity can carry their configuration over rather than rebuilding it by hand.
    - This pairs with the VSCode `textMateRules` paste/import path in § Semantic Highlighting & Theme — highlighting config is one slice of the broader "import my IDE settings" capability.
    - → Research: `docs/research/1_DEEP/ide-feature-parity/1_REVIEW.md`

  + **Mount/open a repo from GitHub or a local directory** **(Concept, needs research)**
    - Thot should let a user mount or open a repo — either from GitHub or from a local directory — rather than only opening single files.
    - Comparable apps already do this: Claude Cowork prompts the user to mount a repo on first use; claude.ai/code runs instances each linked to a GitHub repo. As more iPad / iOS coding apps ship, this mount-a-repo setup is becoming the norm.
    - This is the substrate that makes Thot a credible IDE replacement for markdown work: a user could mount their project's repo and edit its markdown (READMEs, planning docs, specs) directly. It pairs with the claude.ai/code-user target market (often on a device without a full IDE — see the IDE-for-Markdown Replacement pillar) and with the iPad-in-bed use case.
    - → Research: `docs/research/1_DEEP/ide-feature-parity/1_REVIEW.md`
...
### Recorded decision — no backwards compatibility for the scratchpad → file-editor transition

When Thot moves from a single scratchpad model toward a real file/document editor, **no backwards compatibility is required** for the transition. Rationale: Sean is the only user; the scratchpad was always understood as a stepping stone, not a permanent shape; and an old tagged GitHub version of the scratchpad still works standalone for anyone who wants that exact behavior. So the file-editor work is free to change the persistence and document model without a migration path for old scratchpad state.

### Open Questions — File Management & I/O

  + **Does v5+ introduce a real document model?** Sidebar, Finder-column-view, tags (`#ProjectTag`), `@mention` linking between notes by title — all surfaced as v3.x / v4.x ideas. **CONFLICT**: The v2.0.0 forward-looking section listed Finder-column navigation as a v3.x deliverable, but every subsequent doc has either deferred this to vNext (columns + sticky-notes scaffold) or left it explicitly open. Current canonical position: vNext direction, not promised.
...
### Current state **(Not implemented)**
  ...
  + Adding accounts is *additive*, not replacement — the account-less local mode must remain a first-class path.
...
### Passkey UX — the experience details **(Concept, needs research)**

Passkey-first is locked (above). What's open is the *UX* of doing passkeys well — the pattern questions below need research, but the intent is clear. The whole point of passkey is eliminating password friction: no difficult setup, any details it needs captured during the first signup, and login driven by device recognition. Studying apps and services already doing this flawlessly is the fastest way to get the pattern right.

  + **Primary, but never harder than the alternatives.** Passkey is clearly the main login method — but it must not be *harder* to use than password or social. If passkey is the recommended path, it has to also be the easiest path.
  + **Account-creation timing — add-after, made frictionless.** Some apps create the account *with* a passkey up front; others let the user add a passkey *after* signup. Add-after is the preferable model for Thot — but only if it's genuinely frictionless. Acceptable ways to make it so: a checkbox during signup that opts the user into passkey creation, or a passkey-creation flow that *also* sets a password at the same time, so the user never has to dig into settings later to finish the job. (Some apps force the user into settings to create a passkey after the fact — that is the failure mode to avoid.)
  + **Cross-device passkeys — one passkey works on the user's other device.** A passkey created on one device should work on the user's other devices; don't force a separate per-device setup. Sean observed Stripe doing exactly this — he had a Mac passkey and an iPhone passkey, and now either works on either device.
  + **Phone-as-authenticator — no native app required.** For users without Touch ID (e.g. a Mac keyboard with no fingerprint sensor), the passkey can live on the phone: the web app prompts "use your phone," the user scans their face on the phone, and is logged in — with no native app needed, even for a pure web app. Sean experienced this on a web app that has no iOS or macOS app at all and required nothing special installed. Thot should support this flow. (In the case Sean saw, the web app also let him fall back to his Mac biometric quickly if he'd rather — both paths available.)
  + **Biometric-agnostic.** The system matches face *or* fingerprint for the same account; the user does not have to set up both. If the device unlocks, the account unlocks — the system recognizes the device is the user's.
  + **UI reference screenshots**: `docs/archive/images/passkey-facebook-1.png` and `passkey-facebook-2.png` (Facebook's passkey UI). Sean will collect more passkey-UI examples over time.
  + → Research: `docs/research/1_DEEP/auth-and-sync/OPTIONS.md`
...
### The desire (clear and unambiguous)
  ...
  + Sean's clarification during v3 testing: "I did mention that it would be helpful if we could edit the same document on mobile and desktop. However that functionality does not work; every browser and app silos contents. This across device note taking could be solved by Saving and Opening files."
...
From the canonical strategic framing:
...
Strategic framing from the IDE-for-Markdown Replacement pillar:
...
And the broader strategic frame from the 2026-04-27 session:
...
The collab-for-markdown research bucket exists to validate the gap as a real product opportunity or show why others have already solved it. Tools named in the research seed:
...
From the locked auth-and-sync research:
...
### Architecture shape (from the locked research)
  ...
  + **Permissions**: the full per-doc role model is still open, but one decision is **locked**: a shared URL opens the document in-app as **read-only by default**. The sharer has an explicit option to grant a specific user editable access. So a link does not hand out write access implicitly; viewing is the default, editing is a deliberate per-user grant. The remaining permission detail (org-scoped workspaces, role tiers beyond viewer/editor) is open; the auth-and-accounts seed names "share-link permissions" as table stakes.
...
From `docs/research/1_DEEP/collab-for-markdown/1_REVIEW.md` — **the bucket is open; these questions are not yet answered**:
...
### Open Questions — Real-Time Collaboration
  ...
  + **Permissions model**: the share-link default is **locked** — a shared URL opens read-only by default, with the sharer granting editable access per specific user (see Architecture shape § Permissions). What remains open: the full per-doc role set (owner / editor / commenter / viewer), whether links carry embedded permissions, and org-scoped (workspace) permissions.
...
### Strategic frame
  ...
  + **Locked terminology — the container is a "workspace"**
    - The top-level container is called a **"workspace"** — deliberately broader than "Projects." Sean's directive: a workspace is the kind of thing a user splits as life / work / school, a larger grouping than a single project. This is a locked naming decision, not an open question; "Projects" is too narrow for the top level.

  + **Locked decision — hierarchy must go deeper than two levels**
    - The workspace hierarchy must support drilling down **more than two levels** deep. A workspace → group → note ladder that bottoms out at two levels is not enough. The Finder-style column navigation (below) exists precisely to make arbitrary-depth drill-down navigable. This is a Sean directive, not a directional idea — depth beyond two levels is a requirement the columns design must satisfy.

### Note operations **(Concept, needs research)**

Before the columns UI, the post-its, or the tag system, a multi-note workspace needs the basic primitives this section otherwise never spells out:

  + **Create / delete / rename notes** — the elementary lifecycle operations. Rename is not trivial: a note rename must propagate to every `@noteName` link that references it, so renaming requires a workspace-level title index and reference graph (the same machinery `@noteName` already depends on — see Interactive Markdown § `@noteName`).
  + **Note ordering, pinning, favorites** — manual ordering of notes within a group, pinning a note to the top, marking notes as favorites for quick access. Lightweight surfacing, not enforced taxonomy.
  + **Search across all notes** — full-text search spanning every note in the workspace. This is entirely absent today (Thot has only the per-document CM6 Find HUD). A multi-note product is not credible without it; search across notes should be treated as a first-class workspace primitive, not a later polish item.

These are substantive enough to scope deliberately — they are the floor the columns/post-it/tag features build on, and none of them can be assumed to "come for free" with the columns UI.
...
### Finder-style column navigation **(Concept, needs research)**
  ...
  + **Open questions from `COLUMNS_LAYOUT.md`**
    ...
    **§ 1. The layout itself — what does the screen look like?**
    ...
    - **AWAITING SEAN'S FILL-IN**

### Post-it preview snippets per note **(Concept, needs research)**

  + **v1 / v2 origin**
    - *"Configurable 'post-it' preview snippets per note"*
    - *"Instead of auto preview like 'Notes' app (and only one awkward column in Apple Notes app): Configurable 'post-it' preview snippets per note so you can preview what you want specifically so that, at a high level the app works as a quick reference tool, but the drill down and multi layers makes it a really powerful planning tool."*
  ...
  + **At-a-glance + drill-down model**
    - *"at a high level the app works as a quick reference tool, but the drill down and multi layers makes it a really powerful planning tool"*
  ...
  + **Native surface extensions**
    ...
    - *"Our UI layout allowing users to set 'post-it' visible text — It could extend that feature to even more helpful locations — Lock-screen notes, etc."*
...
### AI-populated detail in columns **(Concept, needs research)**
  ...
  + **Origin**
    - *"post-it visible text"* with *"AI populated details in some cases"* (also cross-referenced in `COLUMNS_LAYOUT.md`)
  ...
  + **Visual distinction**
    ...
    - Per `COLUMNS_LAYOUT.md`: *"Is there a visual distinction between user-created and AI-created notes?"* (open)
...
From `docs/research/1_DEEP/feature-research/COLUMNS_LAYOUT.md`:
...
### Persistence implications
  ...
  + Cross-device sync (vNext) requires server-side backing — Vercel Postgres / Neon per the v4 roadmap.
...
### Strategic frame
  ...
  + **Web app is canonical**
    ...
    - *"The PWA will be the most robust version of Thot, with all the features that people would expect from a modern markdown editor. The wrapper would be bit more than a novelty because we can add additional features that are *only* available in that version. We can charge more for the app store version"*
    - **Status**: locked direction. *"We would focus on building the web app first."*
  ...
  + **Don't promote PWA install**
    ...
    - The decision: suppress the PWA install button/banner entirely. Real native shells come in vNext; an installable PWA would muddy that upgrade narrative.
    - **A concrete technical reason, beyond "users dislike PWAs"**: an installed PWA's lifecycle is tied to its host browser process. The PWA opens chromeless and looks like an app, but quitting the host browser (e.g. quitting Chrome) kills the PWA along with it. Basing the install path on a specific browser engine is also a strategic risk amid browser-market churn — the "installed app" would inherit whatever happens to that browser. Real native shells own their own process and don't carry that dependency.
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md` (verbatim):
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md`:
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md`:
  ...
  + **Why through WKWebView is not enough (§ 7.2 verbatim)**
    ...
    - *"Why it's insufficient for Thot: **Latency.** Native PencilKit uses Apple's predictive low-latency pipeline (~9ms perceived). WKWebView pointer events route through JS + DOM and land closer to 60–120ms. Users notice immediately; serious annotation feels broken."*
  ...
  + **The shape that works**
    - *"A native `PKCanvasView` overlay above the WKWebView, bound to an 'annotate mode' toggle. When on, WKWebView interaction disables; PencilKit captures strokes; Vision OCRs; results post back to the web layer as text edits or as a stored ink layer on the document. ~200–500 lines of Swift plus a small Capacitor plugin for the toggle/OCR mediation."*

  + **Sean's framing**
    - *"And I am also very much an iPadOS Apple Pencil lover — Apple Notes does it too but they're so clunky and it goes no where — Ours would be more like actual feedback being given on a document with a red pen — We'd offer Standard blue/black/red pen or black Sharpie — Offer to convert handwritten messages to actual text"*
    - *"Here's some value! — Apple Notes does it too but that app is so clunky — We'll consciously be simple in how we integrate tools like this — Standard blue/black/red pen or black Sharpie — Also offer to convert handwritten messages to actual text — Let you red pen edit a document"*
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md`:
  ...
  + **Transcription provider open question**
    - *"`SFSpeechRecognizer` adequate, or does audio need to round-trip to phone/server for Whisper-quality? Latency vs quality."*
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md`:
  ...
  + **Notarized DMG distribution (no App Store)**
    - From Native Apps research: *"Native-without-App-Store paths — does every macOS app require App Store submission and a paid developer account? Or can a SwiftUI wrapper be distributed as a notarized DMG outside the store? (For macOS yes; for iOS no.)"*
...
#### Live Activities & Widgets
  ...
  + *"Live Activities & Widgets — This has potential — Our UI layout allowing users to set 'post-it' visible text — It could extend that feature to even more helpful locations."*
  + *"Full, immediate integration with Widgets, Lock Screen Live Activities, and App Intents for Shortcuts — Glanceable data on the Lock Screen, Home Screen, and Dynamic Island."*
...
#### Spotlight search integration **(Concept)**

  + A native-shell affordance currently absent: Thot notes should be findable via macOS / iOS **Spotlight search**. A user searching their device should see Thot notes surface in results alongside everything else the system indexes.
  + Implementation: `CoreSpotlight` — index each note as a `CSSearchableItem` (title + content snippet), kept fresh as notes change via the App Group container the native side already reads. Tapping a result deep-links into the note. Native-only; the PWA layer cannot reach the system index.

#### Quick Look preview for `.md` files **(Concept)**

  + A second absent native-shell affordance: previewing a Thot markdown file in Finder via the spacebar **Quick Look** preview. Select a `.md` file in Finder, press space, and see a clean rendered preview without opening the app.
  + Implementation: a Quick Look extension (`QLPreviewProvider` / Thumbnail extension) bundled with the macOS native shell, rendering the markdown to the same clean-prose look the print/export mode targets (see § Counters & Productivity § Print to Simple PDF). Native-only; ships with the macOS shell phase.
...
#### Writing Tools (iOS 18+)
  ...
  + *"Apple Intelligence-powered writing features — Proofread, refine, rewrite, summarize are available in PWA with right click — But iOS 18+ Writing Tools supposedly are only available native"*
...
#### Biometric (Face ID / Touch ID)
  ...
  + *"Biometric Security — Create locked sections or notes — Keep your diary truly personal — Actually this would be sort of essential in that we'd be setting up passkey login anyway; I'm assuming that the native app biometric would just make that faster and just not a login screen."*
...
#### Share Sheet
  ...
  + *"'share sheet' — Send Apple notes app, etc. text to Thot, and vice versa; send the text to any app, messages, etc. — Could this be fashioned into an 'Import my notes from Apple Notes' onboarding feature?"*
...
#### Native file system access
  ...
  + *"file system access would be there"* (from the v2 SwiftUI wrap-up)
...
#### System-Level Integrations (Siri, Maps, Calendar, Camera, Contacts)
  ...
  + *"System-Level Integrations — Direct interaction with Siri, Apple Maps, Calendar, Camera flash, and Contacts without permission friction — Background processing; e.g., syncing, location tracking in the background; 'location tracking' always gets my attention"*
  + Most are out-of-scope for v1 native. Calendar / Reminders integration is in-scope (see below).

#### Advanced Haptics & UI

  + *"Advanced Haptics & UI — Custom haptic feedback (Haptic Engine) — System-level animations that create a 'signature Apple feel'"*
  + Polish; not a feature differentiator. Save the cycles for Phase B–E core features.

#### Platform-Specific UI Components

  + *"Platform-Specific UI Components — Accessing exclusive SwiftUI views and modifiers like NavigationSplitView for iPad/Mac — Material backgrounds, and SwiftCharts"*
...
#### Push Notification Reliability
  ...
  + *"Push Notification Reliability — Consistently reliable push notifications without browser dependence — To be fair, I've literally never gotten a PWA notification before"*
...
### visionOS — speculative future watching
  ...
  + **Origin**
    - *"Spatial Computing (visionOS) — This just needs to be on the radar — Creating native visionOS apps with 3D elements, volumetric windows, and RealityKit integration — People could probably 'Go to the beach to write' 'Sit under a tree and draw' — I've been fascinated with the idea of development in visionOS"*
...
### Rich note metadata — speculative (context auto-enrichment)

  + **The concept** **(Speculative — future watching)**
    - Notes could carry rich metadata the way photographs do — location, time, surrounding context — captured alongside the text rather than as something the user types.
    - Depending on how mature Apple Intelligence becomes, the app could auto-pull contextual data from the user's calendar events, photos, and nearby-friend proximity to enrich a note: a note written during a calendar event could quietly know which event, a note written somewhere could know where, who was nearby, what photos were taken around then.
    - Geo-tagging alone is already on-doc (it appears in Phase C's `ActivityAttributes`); this is the broader concept — a note as a context-rich object, not just text plus a coordinate.
    - Far-horizon and gated on Apple Intelligence capability; kept on the radar.

### Apple Wallet — speculative (LLM token purchases)

  + **Origin**
    - *"Apple Wallet — Because how cool would it be to be the person who made it normal to buy things from your notes app — IDK what yet tho lol ... LLM tokens haha"*
...
### macOS-specific affordances
  ...
  + **macOS menubar jot pad (Phase F or separate track)**
    - *"macOS toolbar/notch jot pad — Sean's 'lives in the menubar, hover to capture' idea. Quick-capture surface that doesn't context-switch the user out of their workflow. SwiftUI menubar apps are well-supported."*
...
From `docs/research/1_DEEP/native-wrapper/OPTIONS.md` (Phase 2 should target these directly):
...
### Build caution — verify SwiftUI against current docs

LLMs tend to generate outdated SwiftUI and Apple-framework code — APIs that were deprecated, renamed, or replaced by newer modifiers. Any native Swift work (PencilKit overlay, WatchOS target, App Intents, WidgetKit widgets, the menubar jot pad) must be verified against current 2026 SwiftUI / framework documentation rather than trusting AI-generated code as written. This applies to every native phase (B–F) and is a standing build note, not a one-time check — re-verify each time native code is generated.

### MCP Server Integration **(Concept, needs research)**

Using MCP (Model Context Protocol) tools as bridges where Apple restricts API access. The concrete case: reaching Apple Notes data — which Thot's WKWebView/Capacitor layer cannot touch directly — via a local MCP bridge that exposes that data through a tool interface. More broadly, MCP servers could be a way to future-proof system integration: where a platform closes off a native API, an MCP bridge becomes the integration seam. Needs research on what is actually reachable this way and what the security/permissions story looks like.
...
This is the business-track section; it stays brief. The detailed work happens in a parallel track (see "Parallel Track — Business Plan Kickoff" below). What's captured here is the seed-question surface from `docs/research/business/monetization/1_REVIEW.md`, the free-tier UX constraint, the trust posture, and the Stripe AI billing coupling — enough that future business-plan work doesn't have to re-derive the framing.

### Strategic frame

  + **Why monetization needs to be decided before public launch**
    - *"Thot is approaching the public-release line (v5.0). A real business plan — positioning, audience tiers, monetization model, competitive landscape, GTM — needs to be in place before the soft launch, not improvised at it."*
    - *"Sean has noted that the recent shift away from Cursor / Antigravity back to terminal-native AI coding has clarified Thot's positioning specifically, which is signal that the messaging surface is ready to be formalized."*

  + **Sean's instinct (verbatim)**
    - *"Sean's instinct: the free tier must remain genuinely useful (not a 3-day trial with paywalls); the paid tier earns its money by unlocking real value (sync, collab, AI affordances)."*

  + **Historical anchor — the v3-era pricing model (superseded)**
    - An early v3-era model floated roughly **$3–5/month** and gated file operations plus multiple windows behind the paid tier.
    - This is **superseded**. The current model treats file operations as free/shipped (Open / Save As are baseline editor features) and draws the paid line at cross-device sync, AI affordances, and multi-user collaboration.
    - The $3–5/mo figure is kept only as a historical anchor — a data point on where Sean's pricing intuition started, not a current recommendation.

  + **Naming decision — "thots"**
    - The product name is **"thots"** — lowercase in casual contexts, "Thots" in formal docs.
    - This affects the `package.json` name, the domain (already `thots.august.style`), the docs, and marketing.
    - The decision: defer the rename until after launch — so the codebase and docs keep "Thot" for now, with the "thots" rename queued as a post-launch task rather than a pre-launch churn.

### Seed questions awaiting business-plan research

From `docs/research/business/monetization/1_REVIEW.md` (verbatim):
  ...
  + **Stripe AI billing for token usage**
    ...
    - Cross-reference: *"Stripe billing has LLM token that adjusts for model."*
...
### Free-tier UX constraint
  ...
  + **The constraint (verbatim)**
    - *"Sean's instinct: the free tier must remain genuinely useful (not a 3-day trial with paywalls)"*
...
### Trust / sovereignty posture
  ...
  + **From the bucket**
    - *"Sean's privacy/sovereignty lineage."*
...
### Competitive pricing context (placeholder)
  ...
  + **`writer-computer`** (github.com/joelbqz/writer-computer) — a free, open-source writing app flagged for competitive analysis. Notable for what it is *not*: it is not a monospace editor and it does not do syntax highlighting. Worth examining as a contrast point — a free OSS writing tool that deliberately omits the two things Thot treats as core.

### Parallel track — Business Plan Kickoff

The monetization and positioning work is a distinct **parallel track**, run alongside (not inside) the product-spec work. Its first session has four deliverables:

  1. **A scope statement** — what the business-plan track does and does not cover.
  2. **A gap audit of existing research** — what monetization / market-positioning research already exists versus what is missing.
  3. **A research plan** — the sequence of research the track needs to run.
  4. **A `docs/BUSINESS_PLAN.md` skeleton** — the document the track fills in over time.

Business-track research now lives at `docs/research/business/` (its own sub-tree, separate from the product-feature research buckets). Promotion of this track — moving it from queued to active — is **Sean-gated**: it advances only when Sean says so, not automatically off the back of product-spec progress.

### How this section interacts with product spec

  + Monetization is a *business-track* concern; this section flags it for the parallel business-plan track.
